...
class Agent(BasicAgent):
    # class variables for now
    SIGNIFICANT = .25 #25% more or less is "significant
    SIG_IMBALANCE = .33
    LOW_INVENTORY = .1 #10% of ideal inventory = "LOW"
    HIGH_INVENTORY = 2.0 #200% of ideal inventory = "High"
    
    def __init__(self, id, data):
        """ id takes int, data takes AgentData object"""
        
        super.__init__(self, id, data)
    
    def create_bid(self, bazaar, good, limit):
        """
        bazaar: Market object
        good: String
        limit: float
        """
        bid_price = determine_price_of(good)
        ideal = determine_purchase_quantity(bazaar, good)
        ...
        if quantity_to_buy:
            return Offer(id, good, quantity_to_buy, bid_price)
